...
import type { PracticeResource } from "@/lib/practice-tools";

type PracticeResourceCardProps = Pick<
  PracticeResource,
  "title" | "url" | "description" | "type"
>;

export function PracticeResourceCard({
  title,
  url,
  description,
  type,
}: PracticeResourceCardProps) {
  const buttonLabel =
    type === "generator" ? "Open Practice Tool" : "Open Problem Set";

  return (
    <ExternalLink
      href={url}
      aria-label={`${buttonLabel}: ${title} (opens in a new tab)`}
      className="group flex min-h-60 flex-col rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_5px_18px_rgba(15,23,42,0.045)] transition duration-200 hover:-translate-y-0.5 hover:border-emerald-800/25 hover:shadow-[0_10px_26px_rgba(15,23,42,0.07)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-700 focus-visible:ring-offset-3 motion-reduce:transform-none motion-reduce:transition-none"
    >
